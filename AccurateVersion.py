...
class Agent:
    def __init__(self, position):
        ...
        self.heading = self.find_heading((self.velocity))
        # Check this
        self.angle_to_com, 
        self.distance_to_com,
        self.speed_diff = env.compute(self)
        print(env.compute)
...
class FlockingEnv(gym.Env):
    def __init__(self):
        ...
        min_per_agent = [
            -np.inf, -np.inf,  # x, y
            -2.5, -2.5,        # vx, vy
            -2.5, -2.5,        # heading_x, heading_y
            -np.pi,            # angle_to_com ([-π, π])
            0.0,               # distance_to_com ([0, ∞))
            -2.5               # speed_diff
        ]
        ...
        min_obs = np.array(min_per_agent * len(self.agents), dtype=np.float32)
        max_obs = np.array(max_per_agent * len(self.agents), dtype=np.float32)
    ...
    def reset(self):   
        ...
        for agent in self.agents:
            ...
            self.heading = self.find_heading((agent.velocity))
            self.angle_to_com, 
            self.distance_to_com,
            self.speed_diff = self.compute(agent)
            print(env.compute)
    ...
    def simulate_agents(self, actions):
        ...
        for i, agent in enumerate(self.agents):
            position, velocity, heading = agent.update(actions_reshaped[i])
            angle_to_com, distance_to_com, speed_diff = self.compute(agent)
            observation_pair = np.concatenate([position, velocity, heading, angle_to_com,
                                              distance_to_com, speed_diff])
    ...
    def get_observation(self):
        ...
        obs = np.zeros((n_agents, 9), dtype=np.float32)

        for i, agent in enumerate(self.agents):
            obs[i, 0] = agent.position[0]         # x
            obs[i, 1] = agent.position[1]         # y
            obs[i, 2] = agent.velocity[0]         # vx
            obs[i, 3] = agent.velocity[1]         # vy
            obs[i, 4] = agent.heading[0]          # heading_x
            obs[i, 5] = agent.heading[1]          # heading_y
            obs[i, 6] = agent.angle_to_com        # angle_to_com
            obs[i, 7] = agent.distance_to_com     # distance_to_com
            obs[i, 8] = agent.speed_diff  
...
policy = "MlpPolicy"
ModelName = rf"{Files['Flocking']}\\Models\\Flocking_{policy}_{SimulationVariables['LearningTimeSteps']}"
...
model.save(rf"{Files['Flocking']}\\Models\\Flocking_'{policy}_Simulation'")
