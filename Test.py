...
def generateVelocityCombined(episode, episode_folder):
    velocities_dict = {}
    ...
    for agent_id in range(3):
        ...
        plt.plot(velocities_magnitude, label=f"Agent {agent_id+1}", color=colors[agent_id], linewidth=0.5)

    plt.xlabel("Time Step")
    plt.ylabel("Velocity Magnitude")
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(episode_folder, f"Episode{episode}_CombinedVelocity.png"))
    plt.close()  # Close the figure to free memory

def generateAccelerationCombined(episode, episode_folder):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(episode_folder, "Combined_Acceleration.png"))
    plt.close()  # Close the figure to free memory
...
def generateVelocity(episode, episode_folder):
    ...
    for agent_id in range(3):
        plt.figure(figsize=(10, 5))
        plt.clf()
        agent_velocities = np.array(velocities_dict.setdefault(agent_id, episode_velocities.get(str(agent_id), [])))
        agent_velocities = savgol_filter(agent_velocities, window_length=4, polyorder=3, axis=0)
        velocities_magnitude = np.sqrt(agent_velocities[:, 0]**2 + agent_velocities[:, 1]**2)
...
def generateAcceleration(episode, episode_folder):
    ...
    for agent_id in range(3):
        plt.figure(figsize=(10, 5))
        plt.clf()
        agent_accelerations = np.array(episode_accelerations.get(str(agent_id), []))
        smoothed_accelerations = np.sqrt(agent_accelerations[:, 0]**2 + agent_accelerations[:, 1]**2)
        smoothed_accelerations = savgol_filter(smoothed_accelerations, window_length=15, polyorder=3, axis=0)
        plt.plot(smoothed_accelerations[::downsample_factor], label=f"Agent {agent_id+1}", color=colors[agent_id], linewidth=0.5)
        plt.xlabel("Time Step")
...
def generatePlots():
    for episode in range(SimulationVariables["Episodes"]):
        episode_folder = setup_episode_folder(f"Episode{episode}")
        generateVelocityCombined(episode, episode_folder)
        generateVelocity(episode, episode_folder)
        generateAccelerationCombined(episode, episode_folder)
...
for episode in tqdm(range(0, SimulationVariables['Episodes'])):
    ...
    while timestep < min(SimulationVariables["EvalTimeSteps"], 1500):
